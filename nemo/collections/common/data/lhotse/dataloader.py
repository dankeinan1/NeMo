...
class MultimodalSamplingConstraint(SamplingConstraint):
    ...
    def measure_length(self, example: Any) -> float:
        if isinstance(example, Cut):
            ...
            text_tokens = _measure_tokens(example, allow_none=True)
...
def _measure_tokens(cut: Cut, allow_none: bool = False) -> int:
    ...
    if len(supervisions_with_tokens) > 0 or allow_none:
